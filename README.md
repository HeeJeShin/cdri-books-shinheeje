...
4. **반응형 목록 — 데스크톱 페이지네이션 / 모바일 무한 스크롤** — 페이지당 10건 공통. 데스크톱은 페이지네이션(현재 페이지 URL `?page=` 동기화, `Pagination` 공통 컴포넌트), **모바일(<768px)은 무한 스크롤**(IntersectionObserver)로 뷰포트에 따라 자동 전환.
